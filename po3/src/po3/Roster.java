...
public class Roster {
     ...
     *
     * PSEUDOCODE: method addStudent(pStudent : Student) : void add (will
     * append) pStudent to mStudentList end method
     */
     ...
     *
     * PSEUDOCODE: method getStudent(pLastName : String) : Student -- Get the
     * index of the student in the student list index = call
     * Searcher.search(getStudentList(), pLastName) -- If index is -1 then no
     * student with that last name could be found so we return -- null.
     * Otherwise, we get the Student from the student list at the index and
     * return -- the Student. if index == -1 then return null else return the
     * Student object in getStudentList() at index 'index' end getStudent
     */
     ...
     *
     * Note: it is extremely sleazy to provide public access to the entire
     * private student list (mStudentList) in this way because it gives whoever
     * calls this method the ability to modify any Student in the roster. It
     * would be better to have the Roster class implement an iterator that would
     * permit other objects to iterate over the elements of the list, but in an
     * effort to keep the project as simple as possible, I am taking the sleazy
     * route.
     *
